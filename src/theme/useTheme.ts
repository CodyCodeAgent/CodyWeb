...
function applyCurrentTheme(): void {
  ...
  root.style.setProperty('--skin-background-blur', `${resolvedSkin.value.background?.blur ?? 0}px`)
  root.style.setProperty('--skin-background-dim', `${resolvedSkin.value.background?.dim ?? 30}%`)
  root.style.setProperty('--skin-background-saturation', `${resolvedSkin.value.background?.saturation ?? 100}%`)
